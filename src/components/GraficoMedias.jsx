...
/* Médias mensais de Faturamento, Despesa, RH e Resultado, ao lado do
   ...
   ideia é sempre mostrar onde o último mês está frente à média; a
   explicação desse % fica na nota de rodapé. Título centralizado e os 4
   itens distribuídos por toda a altura do cartão (mesma altura do
   gráfico de evolução, ao lado) — pra não sobrar espaço em branco
   embaixo quando o gráfico principal cresce. */
const fmtCompacto = (v) => {
...
export default function GraficoMedias({ meses }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 800, fontSize: 13.5, color: C.navy, marginBottom: 2, textAlign: 'center' }}>
        Médias mensais
      </div>
      <p style={{ fontSize: 11, color: C.prata, marginBottom: 0, textAlign: 'center' }}>Média de {n} {n === 1 ? 'mês' : 'meses'} no período.</p>
      ...
      <p style={{ fontSize: 10, color: C.prata, textAlign: 'center', margin: 0, paddingTop: 8, borderTop: `1px solid ${C.bgLeve}` }}>
        % = variação do último mês ({mesLabel(ultimoMes.competencia)}) em relação à média do período.
      </p>
    </div>
  );
}
